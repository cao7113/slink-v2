defmodule SlinkWeb.Api.LinkControllerTest do
  ...
  describe "Get link with unauthenticated connection" do
    test "renders link when data is valid", %{conn: conn, unauthed_conn: unauthed_conn} do
      conn = post(conn, ~p"/api/links", link: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(unauthed_conn, ~p"/api/links/#{id}")
